...
uniform vec3 viewPos; // You will need to pass the camera/view position to the shader
...
void main()
{
    ...
    vec3 ambient = ambientStrength * vec3(1.0, 1.0, 1.0); // Assuming white light

    // Diffuse Component
    float diffStrength = max(dot(normal, lightDir), 0.0);
    vec3 diffuse = diffStrength * vec3(1.0, 1.0, 1.0); // Assuming white light

    // Specular Component
    float specularStrength = 0.5;
    vec3 reflectDir = reflect(-lightDir, normal); 
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), 32.0);
    vec3 specular = specularStrength * spec * vec3(1.0, 1.0, 1.0); // Assuming white light

    // Combine results
    vec3 result = (ambient + diffuse + specular) * texture2D(tex, vTexCoord).rgb;
